...
func _ready() -> void:
	super._ready()
	front_face_texture.custom_minimum_size = card_size
	back_face_texture.custom_minimum_size = card_size
	if front_image:
...
func set_faces(front_face_: Texture2D, back_face_: Texture2D) -> void:
	front_face_texture.texture = front_face_
	back_face_texture.texture = back_face_
...
func _enter_state(state_: DraggableState, from_state_: DraggableState) -> void:
	super._enter_state(state_, from_state_)
...
func init_tokens() -> void:
	var types = card_info.token_types.split(",")
	var values = card_info.token_values.split(",")
	%SuitTextureRect.texture = load("res://entities/bank/token/images/" + card_info.suit + ".png")
...
func add_token(type_: String, value_: int) -> void:
