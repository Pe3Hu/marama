...
func change_state(new_state_: DraggableState) -> bool:
	if new_state_ == current_state: return true
	
	# Validate transition is allowed by state machine rules
	if not new_state_ in allowed_transitions[current_state]: return false
	...
	# Enter new state
	_enter_state(new_state_, old_state)
	
	return true


# Handle state entry
func _enter_state(state_: DraggableState, from_state_: DraggableState) -> void:
	match state_:
		DraggableState.IDLE:
		...
		DraggableState.HOLDING:
			...
			if from_state_ == DraggableState.HOVERING:
...
# Handle state exit
func _exit_state(state_: DraggableState) -> void:
	match state_:
		DraggableState.HOVERING:
			z_index = stored_z_index
...
func _update_hover_position(position_: Vector2) -> void:
	current_hover_position = position_
...
func _on_gui_input(event_: InputEvent) -> void:
	if not can_be_interacted_with: return
	
	if event_ is InputEventMouseButton:
		_handle_mouse_button(event_ as InputEventMouseButton)
...
func move(target_destination_: Vector2, degree_: float) -> void:
	# Skip if current position and rotation match target
	if global_position == target_destination_ and rotation == degree_: return
	...
	self.target_destination = target_destination_
	self.target_rotation = degree_
	
	# Initial setup
	rotation = 0
	destination_degree = degree_
...
func _handle_mouse_button(mouse_event_: InputEventMouseButton) -> void:
	if mouse_event_.button_index != MOUSE_BUTTON_LEFT: return
	
	# Ignore all input during MOVING state
	if current_state == DraggableState.MOVING: return
	
	if mouse_event_.is_pressed():
		_handle_mouse_pressed()
	
	if mouse_event_.is_released():
