...
func _ready() -> void:
	if not _pre_process_exported_variables(): return
...
func _add_card_container(id_: int, card_container_: CardContainer) -> void:
	card_container_dict[id_] = card_container_
	card_container_.debug_mode = debug_mode


func _delete_card_container(id_: int) -> void:
	card_container_dict.erase(id_)


# Handles dropped cards by finding suitable container
func _on_drag_dropped(cards_: Array) -> void:
	if cards_.is_empty(): return
	...
	for key in card_container_dict.keys():
		...
		var result = card_container.check_card_can_be_dropped(cards_)
		if result:
			...
			card_container.move_cards(cards_, index)
			return
...
func _add_history(to_: CardContainer, cards_: Array) -> void:
	var from = null
	var from_indices = []
	...
	for _i in range(cards_.size()):
		var card = cards_[_i]
		var current = card.card_container
		if _i == 0:
			from = current
		else:
			if from != current:
		...
		if from != null:
			var original_index = from._held_cards.find(card)
			if original_index == -1:
	...
	history_element.to = to_
	history_element.cards = cards_
	history_element.from_indices = from_indices
	history.append(history_element)


func _is_valid_directory(path_: String) -> bool:
	var dir = DirAccess.open(path_)
	return dir != null
