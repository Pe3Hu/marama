...
func create_card(card_name_: String, target_: CardContainer) -> Card:
	# Use cached data for optimal performance
	if preloaded_cards.has(card_name_):
		var card_info = preloaded_cards[card_name_]["info"]
		var front_image = preloaded_cards[card_name_]["texture"]
		return _create_card_node(card_info.name, front_image, target_, card_info)
	else:
		# Load card data on-demand (slower but supports dynamic loading)
		var card_info = _load_card_info(card_name_)
		if card_info == null or card_info == {}:
			push_error("Card info not found for card: %s" % card_name_)
			return null
		
		# Validate required JSON fields
		if not card_info.has("front_image"):
			push_error("Card info does not contain 'front_image' key for card: %s" % card_name_)
			return null
...
func _load_card_info(card_name_: String) -> Dictionary:
	var json_path = card_info_dir + "/" + card_name_ + ".json"
	if !FileAccess.file_exists(json_path): return {}
...
func _load_image(image_path_: String) -> Texture2D:
	var texture = load(image_path_) as Texture2D
	if texture == null:
		push_error("Failed to load image resource: %s" % image_path_)
		return null
	return texture
...
func _create_card_node(card_name_: String, front_image_: Texture2D, target_: CardContainer, card_info_: Dictionary) -> Card:
	var card = _generate_card(card_info_)
	
	# Validate container can accept this card
	if !target_._card_can_be_added([card]):
		print("Card cannot be added: %s" % card_name_)
		card.queue_free()
		return null
	
	# Configure card properties
	card.card_info = card_info_
	card.card_size = card_size
	
	# Add to scene tree and container
	var cards_node = target_.get_node("Cards")
	cards_node.add_child(card)
	target_.add_card(card)
	
	# Set card identity and textures
	card.card_name = card_name_
	card.set_faces(front_image_, back_image)
