...
func init(_parent: Node, accept_types_: Array =[]):
	parent = _parent
	self.accept_types = accept_types_
...
## @param size_: Size of the sensor area
## @param position_: Position offset from DropZone origin
## @param texture_: Optional texture for sensor visualization
## @param visible_: Whether sensor texture is visible (deprecated)
func set_sensor(size_: Vector2, position_: Vector2, texture_: Texture, visible_: bool):
	sensor_size = size_
	sensor_position = position_
	stored_sensor_size = size_
	stored_sensor_position = position_
	sensor_texture = texture_
	sensor_visible = visible_
...
## @param size_: New temporary sensor size
## @param position_: New temporary sensor position
func set_sensor_size_flexibly(size_: Vector2, position_: Vector2):
	sensor_size = size_
	sensor_position = position_
...
func change_sensor_position_with_offset(offset_: Vector2):
	sensor_position = stored_sensor_position + offset_
...
func set_vertical_partitions(positions_: Array):
	vertical_partition = positions_
	...
	for _i in range(vertical_partition.size()):
		var outline = ReferenceRect.new()
		outline.editor_only = false
		outline.name = "VerticalPartition" + str(_i)
		...
		var local_x = vertical_partition[_i] - global_position.x
...
func set_horizontal_partitions(positions_: Array):
	horizontal_partition = positions_
	# clear existing outlines
	for outline in sensor_partition_outlines:
		outline.queue_free()
	sensor_partition_outlines.clear()
	for _i in range(horizontal_partition.size()):
		var outline = ReferenceRect.new()
		outline.editor_only = false
		outline.name = "HorizontalPartition" + str(_i)
		outline.z_index = CardFrameworkSettings.VISUAL_OUTLINE_Z_INDEX
		outline.border_color = CardFrameworkSettings.DEBUG_OUTLINE_COLOR
		outline.mouse_filter = Control.MOUSE_FILTER_IGNORE
		outline.size = Vector2(sensor.size.x, 1)
		var local_y = horizontal_partition[_i] - global_position.y
...
func get_vertical_layers() -> int:
	if not check_mouse_is_in_drop_zone(): return -1

	if vertical_partition == null or vertical_partition.is_empty(): return -1
	...
	for _i in range(vertical_partition.size()):
		if mouse_position.x >= vertical_partition[_i]:
			current_index += 1
		else:
			break
...
func get_horizontal_layers() -> int:
	...
	if horizontal_partition == null or horizontal_partition.is_empty(): return -1
	...
	for _i in range(horizontal_partition.size()):
		if mouse_position.y >= horizontal_partition[_i]:
			current_index += 1
		else:
			break
	return current_index
